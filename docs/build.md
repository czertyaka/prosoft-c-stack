...
Очень желательно после внесения изменений применить
автоформатирование по правилам, определенным в [``.clang-format``](./.clang-format).
Для этого потребуется установить утилиту `clang-format`.
Конфигурация совместима с версией 19 и старше:
...
Для этого перед коммитом выполните:

```sh
clang-format -i cstack.*
```
