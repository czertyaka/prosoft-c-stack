...
static unsigned int handlers_max; //УДАЛИТЬ ЕСЛИ НЕ ПОЛУЧИТСЯ!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
hstack_t stack_new(void)
{
    ...
    if (g_table.entries == NULL)
    {
        ...
        if (g_table.entries == NULL)
            return -1;
        handlers_max = 1; //максимум хендлеров увеличился до 1
    }
    ...
    for (int i = 0; i < (int)handlers_max; ++i)
    {
    ...
    {
        handlers_max += 1u;
        handler = (int)handlers_max - 1; //из-за того, что хендлеры идут с 0
        g_table.entries = (stack_entry_t*)realloc(g_table.entries, (handlers_max) * sizeof(stack_entry_t));
        if (g_table.entries == NULL)
            return -1;
    }
...
{
    if (hstack <0 || hstack >= (int)handlers_max || g_table.size == 0u)
        return 1;
...
void stack_free(const hstack_t hstack)
{
    ...
    while (g_table.entries[hstack].top != NULL)
    {
        temp = g_table.entries[hstack].top;
    ...
    //становится равным размеру g_table.size, если g_table.size и handlers_max не совпадают
    if (g_table.size < handlers_max)
    {
        int handler, count = 0;
        for (handler = 0; handler < (int)handlers_max; ++handler)
        {
        ...
        if (handler < (int)(handlers_max-1))
        {
            stack_entry_t *temp_entries = (stack_entry_t *)malloc((handler+1)*sizeof(stack_entry_t));
            if (temp_entries == NULL)
                    return;
            for (int i = 0; i <= handler; ++i)
                temp_entries[i] = g_table.entries[i];
                g_table.entries = (stack_entry_t*)realloc(g_table.entries, (handler+1)*sizeof(stack_entry_t));
                if (g_table.entries == NULL)
                    return;
            for (int i = 0; i <= handler; ++i)
                g_table.entries[i] = temp_entries[i];
            free(temp_entries);
            temp_entries = NULL;
            handlers_max = handler + 1;
        }
    }
    return;
}
